            ...
            for degree in range(2, 7):
                if row < (size - degree):
                    ...
                    if degree == 2:
                        ...
                        cef_hamiltonian[row, column] += (parameters['B22'] * o22 + parameters['B42'] * o42 +
                                                         parameters['B62'] * o62)
                    if degree == 3:
                        o43 = 0.25 * down_operator(n, degree) * (m + n)
                        o63 = (0.25 * (11 * (m ** 3 + n ** 3) - 3 * (m + n) * j_module_square -
                                       59 * (m + n)) * down_operator(n, degree))
                        cef_hamiltonian[row, column] += parameters['B43'] * o43 + parameters['B63'] * o63
                    if degree == 4:
                        o44 = 0.5 * down_operator(n, degree)
                        o64 = (5.5 * (m2 + n ** 2) - j_module_square - 38) * o44
                        cef_hamiltonian[row, column] += parameters['B44'] * o44 + parameters['B64'] * o64
                    if degree == 6:
                        column = row + 6
                        n = m + 6
    ...
    def transition_probabilities(self):
        ...
        for row in range(size):
            ...
            for column in range(row + 1, size):
                m = size - 1 - j
                j_z[row, column] += (eigenfunctions[size - 1, row] * eigenfunctions[size - 1, column] * m)
                for row_j in range(size - 1):
                    m = row_j - j
                    j_z[row, column] += (eigenfunctions[row_j, row] * eigenfunctions[row_j, column] * m)
                    column_j = row_j + 1
                    n = column_j - j
                    common_root = sqrt(j_2 - m * n)
                    j_plus[row, column] += (eigenfunctions[column_j, row] * eigenfunctions[row_j, column] *
                                            common_root)
                    j_minus[row, column] += (eigenfunctions[row_j, row] * eigenfunctions[column_j, column] *
                                             common_root)
        ...
        for row in range(self.eigenvalues.size):
            for column in range(self.eigenvalues.size):
                ...
                if abs(column_value - row_value) < 0.00001 * temperature_as_energy:
                    ...
                    chi_van_vleck['z'] += (2 * j_z_square * current_bolzmann / (column_value - row_value))
                    chi_van_vleck['x'] += (0.5 * (j_plus_square + j_minus_square) * current_bolzmann /
    ...
    def __str__(self):
        ...
        for key in ('z', 'x'):
            if magnetic_moment[key] > 1e-9:
                output.append(f'<J{key}> = {j_average[key]:7.3f}; <mu_{key}> = {magnetic_moment[key]:7.3f} mu_Bohr;')
...
if __name__ == '__main__':
    ...
    w = 1
    x = -1
    cubic_sample = get_object_with_parameters(crystal=f'YNi2: {rare}3+', rare_earth=rare, w=w, x=x)
