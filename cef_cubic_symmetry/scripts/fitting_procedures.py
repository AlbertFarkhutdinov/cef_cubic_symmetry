...
def multi_lorentzian_with_gauss(arg: float, center: float, width: float, amplitude: float,
                                *parameters):
    """Returns value of multi_peak function for lorentzian"""
    return gaussian(arg, center, width, amplitude) + multi_lorentzian(arg, *parameters)
...
def simple_fitting(data: dict):
    ...
    p_opt, p_err = fu.fitting(multi_lorentzian_with_gauss,
                              data,
                              parameters=(*peak_0, 0, *peak_1, *peak_2),
                              min_value=-2,
                              max_value=2)
    fu.print_peak_parameters(multi_lorentzian_with_gauss, p_opt, p_err)
    fitted = multi_lorentzian_with_gauss(data['x'], *p_opt)
    return fitted


if __name__ == '__main__':
    EXPERIMENTAL_DATA = fu.get_data_from_file(join(DATA_PATHS['experiment'],
                                                   'PSI_Tb_YNi2_3meV_1.6K.txt'))
    EXPERIMENTAL_DATA = fu.filtered_data(EXPERIMENTAL_DATA,
                                         min_value=-2,
                                         max_value=2)
