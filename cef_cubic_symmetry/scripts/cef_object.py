...
class CEF:
    ...
    def get_spectrum(self, energies=None, temperature=None, width_dict: dict = None,
        ...
        for peak in peaks:
            if sigma and not gamma:
                spectrum += peak[1] * gauss(energies, peak[0],
                                            sigma)
            elif gamma and not sigma:
                spectrum += peak[1] * lorentz(energies, peak[0],
                                              gamma)
            elif sigma and gamma:
                spectrum += peak[1] * pseudo_voigt(energies, peak[0],
                                                   sigma,
                                                   gamma)
    ...
    def get_chi(self, temperature=None):
        ...
        coefficient = self.material.rare_earth.lande_factor ** 2 / statistic_sum
        chi_curie = {'z': 0, 'x': 0}
