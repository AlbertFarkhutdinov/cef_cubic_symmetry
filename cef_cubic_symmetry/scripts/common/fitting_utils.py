...
def filtered_data(data: dict, min_value=-INFINITY, max_value=INFINITY):
    ...
    rows = [(i, j, k) for (i, j, k) in zip(*data.values())
...
def multi_peak_fitting(function_name: str, data: dict, parameters,
    ...
    """Returns parameters of function fitted to data with several peaks (lorentzian or gaussian)"""
    function = multi_lorentzian if function_name.lower() == 'lorentz' else multi_gaussian
    data = filtered_data(data)
    if (len(parameters) - 1) % 3:
        print('The parameters number does not match peaks number!')
        sys.exit(1)
    else:
        p_opt, p_err = fitting(function,
                               data,
...
if __name__ == '__main__':
    ...
    P_OPT, P_ERR = multi_peak_fitting(function_name='lorentz',
                                      data=DATA,
    ...
    with CustomPlot(data=Data(x=DATA['x'],
                              y_set={
                                  'exp': DATA['y'],
